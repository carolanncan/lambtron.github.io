...
[Artificial neural networks](https://en.wikipedia.org/wiki/Artificial_neural_network) are statistical learning models, inspired by biological neural networks (central nervous systems, such as the brain), that are used in [machine learning](https://en.wikipedia.org/wiki/List_of_machine_learning_concepts). These networks are represented as a system of interconnected "neurons", which send messages to each other. The connections within the network can be systematically adjusted based on inputs and outputs, making them ideal for supervised learning.

Neural networks can be intimidating, especially for people with little experience in machine learning and cognitive science! However, through code, this tutorial will explain how neural networks operate. By the end, you will be able to know how to build your own flexible, learning network.
...
A neural network is a collection of "neurons" with "synpases" connecting them. There are three main parts: the input layer, the hidden layer, and the output layer. Note that you can have _n_ hidden layers, with the term "deep" learning implying multiple hidden layers.
...
Hidden layers are necessary when the neural network has to make sense of something really complicated, contextual, or non obvious, like image recognition. The term "deep" learning came from having many hidden layers. Read more about hidden layers [here](http://stats.stackexchange.com/questions/63152/what-does-the-hidden-layer-in-a-neural-network-compute).

The circles represent neurons and lines represent synpases. Synapses take the input and multiply it by a "weight" (the "strength" of the input in determining the output). Neurons add the outputs from all synpases and apply an activation function.

Training a neural network basically means calibrating all of the "weights" by repeating two key steps, forward propagation and backwards propagation.

In **forward propagation**, we apply a set of weights to the input data and calculate an output. For the first forward propagation, the set of weights are selected randomly.

In **backward propagation**, we measure the margin of error of the output and adjust the weights accordingly to decrease the error.

Neural networks repeat both forward and backward propagations until the weights are calibrated to accurately predict an output.

We'll walk through a simple example of training a neural network to function as an ["Exclusive or" ("XOR") operation](https://en.wikipedia.org/wiki/Exclusive_or) to illustrate each step in the training process.

### Forward Propogation

*Note that all math calculations will show figures truncated to the 1,000th place.*

The XOR function can be represented by the mapping of the below inputs and outputs, which we'll use as training data. Then, it should provide a correct output given any input acceptable by the XOR function.
...
We now assign weights to all of the synapses. Note that these weights are selected randomly (based on Gaussian distribution) since it is the initial forward propagation. The initial weights will be between 0 and 1, but note that the final weights don't need to be.

![](http://i.imgur.com/RRn0pgb.png)

We sum the product of the inputs with set of weights to arrive at the first values for the hidden layer. You can think of the weights as measures of influence the input nodes have on the output.
...
Adding that to our neural network as hidden layer _results_:

![](http://i.imgur.com/GjQuLls.png)

Then, we sum the product of the hidden layer results with a new set of weights to determine the output sum.

```
0.27 * 0.3 + 0.79 * 0.5 + 0.31 * 0.9 = 0.755
```

..then we apply the activation function to get to the final output result..
...
Because we used a random set of initial weights, the value of the output neurons are off the mark; in this case by 0.68. If we stopped here, this set of weights would be a great neural network for inaccurately representing the XOR operation.

Let's fix that by using backward propogation to adjust the weights to improve the network!
...
Similar to forward propagation, backward propagation calculations occur at each "layer". We begin by changing the weights between the hidden layer and the output layer.
...
To calculate the necessary change in the output sum, or `delta output sum`, we take the derivative of the activation function and apply it to the output sum. For our example, our activation function is Sigmoid.

To refresh your memory, the activtion function, Sigmoid, takes the sum and returns the result:
...
Here is a graph of the Sigmoid function to give you an idea of how we are using the derivative to move the input towards the right direction.

![](http://i.imgur.com/3l5PXkJ.jpg)
...
As a reminder, this is the mathematical definition of the `output sum` is the product of the hidden layer result and the weights between the hidden and output layer:
...
This relationship suggests that a greater change in output sum yields a greater change in the weights; input neurons with the biggest contribution (higher weight to output neuron) should experience more change in the connecting synpase.
...
Remember that the relationship between the hidden result, the weights between the hidden and outer layer, and the output sum is:

![](http://i.imgur.com/ITudruR.png)

Instead of deriving for `output sum`, let's derive for `hidden res` as a function of `output sum` to ultimately find out `delta hidden sum`:

![](http://i.imgur.com/25TS8NU.png)
![](http://i.imgur.com/iQIR1MD.png)

Also, remember that the change in the `hidden layer result` can also be defined as:

![](http://i.imgur.com/ZquX1pv.png)

Let's multiply Sigmoid prime of the hidden sum to both sides:
...
Once we get the `delta hidden sum`, we calculate the change in weights between the input and hidden layer by dividing it with the input data, `(1, 1)`. The input data here is equivalent to the `hidden layer results` in the earlier back propagation process to determine the change in the hidden-to-outer weights. Here is the derivation of that relationship, similar to the one before:
...
Just through one iteration of forward and backward propagation, we've already improved the network!!
...
The forward propagation process is a series of sum products and transformations. Let's calculate the first hidden layer sum with all four input data:
...
Then, we do this again with the hidden layer result as the new input to get to the final output result. The entire forward propagation code looks like:
...
Note that `this.activate` and `this.weights` are set at the initialization of a new `Mind` via [passing an `opts` object](https://github.com/stevenmiller888/mind/blob/master/lib/index.js#L36).

### Back Propagation

Back propagation is a bit more complicated. Let's look at the last layer first:

![]()

We calculate the `output sum margin of error` (same equation as before):
...
Then, we do this process [again](https://github.com/stevenmiller888/mind/blob/master/lib/index.js#L125-L126) for the input to hidden layer.
...
Now that we have both the forward and backward propagation, we can define the function `learn` that will put them together. The `learn` function will accept training data (`examples`) as an array of matrices. Then, we assign random samples to the initial weights (via [`sample`](https://github.com/stevenmiller888/sample)). Lastly, we use a `for` loop and repeat `this.iterations` to do both forward and backward propagation.
